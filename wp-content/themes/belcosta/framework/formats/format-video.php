			...
			<?php } else { ?>
				...
				<?php if ( has_post_thumbnail() ) { ?> 	
					<?php $multicheck_value = get_theme_mod( 'insomnia_soc_link', array( 'facebook', 'twitter', 'pinterest' ) ); ?>
						...
						<?php endif; ?>
					<?php if(get_theme_mod('insomnia_post_format', 'enable')) : ?><div class="paper-button-wrap"><div class="paper-button btn-next"><i class="pe-7s-video"></i> <span><?php esc_html_e( "Video", "insomnia" ) ?><span></div></div><?php endif; ?>
				<?php } else { ?>	
					<?php $type_post = '<li><i class="pe-7s-video"></i> <span>' . esc_html__( "Video", "insomnia" ) . '</span></li>'  ?>
				<?php } ?>
				</div>
