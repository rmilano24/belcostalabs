<?php
	$quote_post_data = get_post_meta($post->ID,'insomnia_quote_block',true);
	$quote_post_author = get_post_meta($post->ID,'insomnia_quote_author',true);
	...
	<?php
		...
		$image_url = wp_get_attachment_url($thumbnail_id);		
 		...
 		if(is_single()) {?>
				...
				<div class="quote-format"><?php echo wp_kses_post($quote_post_data); ?></div>
			...
			<?php } else { ?>
				...
				<?php if ( has_post_thumbnail() ) { ?> 	
					<?php $multicheck_value = get_theme_mod( 'insomnia_soc_link', array( 'facebook', 'twitter', 'pinterest' ) ); ?>
						...
						<?php endif; ?>
					<?php if(get_theme_mod('insomnia_post_format', 'enable')) : ?><div class="paper-button-wrap"><div class="paper-button btn-next"><i class="pe-7s-comment"></i> <span><?php esc_html_e( "Quote", "insomnia" ) ?><span></div></div><?php endif; ?>
				<?php } else { ?>	
					<?php $type_post = '<li><i class="pe-7s-comment"></i> <span>' . esc_html__( "Quote", "insomnia" ) . '</span></li>'  ?>
				<?php } ?>
				</div>
