<?php
 	$link_post_link_title = get_post_meta($post->ID,'insomnia_link_title_block',true);
	$link_post_data = get_post_meta($post->ID,'insomnia_link_block',true);
	$link_post_target = get_post_meta($post->ID,'insomnia_link_radio',true);
	$link_post_relationship = get_post_meta($post->ID,'insomnia_link_relationship',true);
...
?>
		...
		$image_url = wp_get_attachment_url($thumbnail_id);		
 		...
 		if(is_single()) {?>
			...
			<div class="link-format"><a href="<?php echo esc_url($link_post_data); ?>"  rel="<?php echo esc_attr($link_post_relationship); ?>"  title="<?php echo esc_attr($link_post_link_title); ?>" target="_<?php echo $link_post_target; ?>" ><?php echo esc_attr($link_post_link_title); ?></a></div>
			</div>
			...
			<?php } else { ?>
				...
				<?php if ( has_post_thumbnail() ) { ?> 	
					<?php $multicheck_value = get_theme_mod( 'insomnia_soc_link', array( 'facebook', 'twitter', 'pinterest' ) ); ?>
						...
						<?php endif; ?>
					<?php if(get_theme_mod('insomnia_post_format', 'enable')) : ?><div class="paper-button-wrap"><div class="paper-button btn-next"><i class="pe-7s-link"></i> <span><?php esc_html_e( "Link", "insomnia" ) ?><span></div></div><?php endif; ?>
				<?php } else { ?>	
					<?php $type_post = '<li><i class="pe-7s-link"></i> <span>' . esc_html__( "Link", "insomnia" ) . '</span></li>'  ?>
				<?php } ?>
				</div>
