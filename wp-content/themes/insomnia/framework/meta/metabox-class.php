...
if ( ! class_exists( 'AT_Meta_Box_Big') ) :
...
class AT_Meta_Box_Big {
  ...
  public function __construct ( $meta_box ) {
    ...
    if (isset($meta_box['use_with_theme']))
      if ($meta_box['use_with_theme'] === true){
        $this->SelfPath = get_stylesheet_directory_uri() . '/framework/meta';
      }elseif($meta_box['use_with_theme'] === false){
        $this->SelfPath = plugins_url( 'meta-box-class', plugin_basename( get_template_directory() ) );
      }else{
  ...
  public function load_scripts_styles() {
    ...
    if (in_array($typenow,$this->_meta_box['pages']) && $this->is_edit_page()){
      ...
      wp_enqueue_style( 'asm-select', $plugin_path . '/css/jquery.asmselect.css' );
      wp_enqueue_style( 'font-awesome', get_template_directory_uri() . '/framework/fonts/font-awesome/css/font-awesome.css' );
      
      // Enqueue Meta Box Scripts
      wp_enqueue_script( 'at-meta-box', $plugin_path . '/js/meta-box.js', array( 'jquery' ), null, true );
      wp_enqueue_script( 'asm-select', $plugin_path . '/js/jquery.asmselect.js', array( 'jquery' ), null, true );
  ...
  public function show_field_repeater( $field, $meta ) {
    ...
    $meta = get_post_meta($post->ID,$field['id'],true);
      ...
      if (count($meta) > 0 && is_array($meta) ){
        ...
        <span class="re-control"><img src="'.$plugin_path.'/images/remove.png" alt="'.__('Remove','insomnia').'" title="'.__('Remove','insomnia').'" id="remove-'.$field['id'].'"></span>
    ...
    echo '" alt="'.__('Add','insomnia').'" title="'.__('Add','insomnia').'" id="add-'.$field['id'].'"><br/></div>';
    ...
    echo '</table><img src="'.$plugin_path.'/images/remove.png" alt="'.__('Remove','insomnia').'" title="'.__('Remove','insomnia').'" id="remove-'.$field['id'].'"></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px; min-height: 50px}
  ...
  public function show_field_color( $field, $meta ) {
    ...
    if( wp_style_is( 'wp-color-picker', 'registered' ) ) { //iris color picker since 3.5
      ...
      echo "<input type='button' class='at-color-select button' rel='{$field['id']}' value='" . __( 'Select a color' ,'insomnia') . "'/>";
  ...
  function Validate_upload_file_type($file) {
    if (isset($_POST['uploadeType']) && !empty($_POST['uploadeType']) && isset($_POST['uploadeType']) && $_POST['uploadeType'] == 'my_meta_box'){
      ...
      if (!in_array($ext, (array)$allowed)){
        $file['error'] = __("Sorry, you cannot upload this file type for this field.", 'insomnia');
        return $file;
      }
  ...
  public function load_textdomain(){
    ...
    load_textdomain( 'insomnia', dirname(__FILE__) . '/lang/' . get_locale() .'mo' );
  }
} // End Class
...
class AT_Meta_Box extends AT_Meta_Box_Big {
   ...
   */
  public function show_field_begin( $field, $meta) {
    echo "<td class='at-field {$field['type']}-field'".(($this->inGroup === true)? " valign='top'": ""). " id='{$field['id']}'>";
    if ( $field['name'] != '' || $field['name'] != FALSE ) {
      echo "<div class='at-label'>";
        echo "<label for='{$field['id']}'>{$field['name']}</label>";
      echo "</div>";
	...
	*/
	...
	$meta = get_post_meta($post->ID,$field['id'],true);
	  ...
	  if (count($meta) > 0 && is_array($meta) ){
		 foreach ($meta as $me){
		   ...
		   if ( in_array( $field['fields'][0]['type'], array('image','file') ) )
			$mmm = $c +1 ;
		   echo '<div class="at-repater-block"><div class="slide-no">Item #'.$mmm.'</div><table class="repeater-table">';
		   if ($field['inline']){
			 echo '<tr class="at-inline" VALIGN="top">';
		   }
		foreach ($field['fields'] as $f){
		...
		if ($field['sortable'])
		  echo '<div class="insomnia_sortable at_re_sort_handle"></div>';
		
		echo '<span class="re-control"><img src="'.$plugin_path.'/images/remove.png" alt="'.__('Remove','insomnia').'" title="'.__('Remove','insomnia').'" id="remove-'.$field['id'].'"></span>';
		echo '</table>';

		echo'<span class="re-control-clear"></span></div>';
		$c = $c + 1;
		}
	  }

	echo '<div class="add-new-item" id="add-'.$field['id'].'"></div><br/></div>';
	...
	echo '<span><img src="'.$plugin_path.'/images/remove.png" alt="'.__('Remove me','insomnia').'" title="'.__('Remove me','insomnia').'" id="remove-'.$field['id'].'" /></span></table></div>';
	$counter = 'countadd_'.$field['id'];
	$js_code = ob_get_clean ();
	$js_code = str_replace("\n","",$js_code);
	$js_code = str_replace("\r","",$js_code);
 ...
 /**
  ...
  public function show_field_image( $field, $meta ) {
    ...
    $has_image    = empty($value['url'])? false : true;
    $PreviewStyle = ( (!$has_image)? "display: none;'": "'");
    $id           = $field['id'];
    $multiple     = isset($field['multiple'])? $field['multiple'] : false;
    $multiple     = ($multiple)? "multiFile " : "";
		echo "<span class='simplePanelImagePreview'><img style='width:80%' src='{$aq_url}' ><br/></span>";
    echo "<input type='hidden' name='{$name}[id]' value='{$value['id']}'/>";
    echo "<input type='hidden' name='{$name}[url]' value='{$value['url']}'/>";
    if ($has_image)
      echo "";
   ...
   * @access public
   */
  public function show_field_popuptextarea( $field, $meta ) {
    $this->show_field_begin( $field, $meta );
      echo "<textarea class='at-textarea large-text".( isset($field['class'])? ' ' . $field['class'] : '' )."' name='{$field['id']}' id='{$field['id']}' ".( isset($field['style'])? "style='{$field['style']}' " : '' )." cols='60' rows='10'>{$meta}</textarea>";
    $this->show_field_end( $field, $meta );
  } 
  ...
  public function show_field_radio( $field, $meta ) {
    ...
    $this->show_field_begin( $field, $meta );
      foreach ( $field['options'] as $key => $value ) {
        echo "<div class='radio-item'><input type='radio' ".( isset($field['style'])? "style='{$field['style']}' " : '' )." class='at-radio".( isset($field['class'])? ' ' . $field['class'] : '' )."' name='{$field['id']}' value='{$key}'" . checked( in_array( $key, $meta ), true, false ) . " /> <span class='at-radio-label'>{$value}</span></div>";
      }
    $this->show_field_end( $field, $meta );
  }
   ...
   * @access public 
   */
  public function show_field_imageradio( $field, $meta ) {
    ...
    $this->show_field_begin( $field, $meta );
		foreach ( $field['options'] as $key => $value ) {
			...
			$checked = '';
			$selected = '';
			if(NULL!=checked( in_array( $key, $meta ), true, false )) {
				$checked = checked( in_array( $key, $meta ), true, false );
				$selected = 'radio-img-selected';  
			}
			
			echo "<span>
				<input id='img_{$field['id']}_{$i}' type='radio' ".( isset($field['style'])? "style='{$field['style']}' " : '' )." class='at-radio img-radio {$key}".( isset($field['class'])? ' ' . $field['class'] : '' )."' name='{$field['id']}' value='{$key}'" . $checked . " /> <div class='dt-radio-label'>{$value}</div>
				<img src='{$plugin_path}/images/{$key}.png' class='of-radio-img {$selected}' onClick='document.getElementById(\"img_{$id}_{$i}\").checked = true;' title='{$value}'/>
				</span>";
		}
    $this->show_field_end( $field, $meta );
  }  
  ...
  public function show_field_checkbox_list( $field, $meta ) {
    ...
    $this->show_field_begin($field, $meta);
      ...
      foreach ($field['options'] as $key => $value) {
        $html[] = "<div class='input-item'><input type='checkbox' ".( isset($field['style'])? "style='{$field['style']}' " : '' )."  class='at-checkbox_list".( isset($field['class'])? ' ' . $field['class'] : '' )."' name='{$field['id']}[]' value='{$value}'" . checked( in_array( $value, $meta ), true, false ) . " /> {$value}</div>";
      }
   ...
   * @access public
   */
  public function show_field_checkbox_list_key ( $field, $meta ) {
    ...
    $this->show_field_begin($field, $meta);
      ...
      foreach ($field['options'] as $key => $value) {
        $html[] = "<div class='input-item'><input type='checkbox' ".( isset($field['style'])? "style='{$field['style']}' " : '' )."  class='at-checkbox_list".( isset($field['class'])? ' ' . $field['class'] : '' )."' name='{$field['id']}[]' value='{$key}'" . checked( in_array( $key, $meta ), true, false ) . " /> {$value}</div>";
      }
  ...
  public function show_field_text( $field, $meta) {  
    $this->show_field_begin( $field, $meta );
    echo "<input type='text' class='at-text".( isset($field['class'])? ' ' . $field['class'] : '' )."' name='{$field['id']}' id='{$field['id']}' value='{$meta}' size='60' ".( isset($field['style'])? "style='{$field['style']}'" : '' )."/>";
    $this->show_field_end( $field, $meta );
  } 
   ...
   * @access public
   */
  public function show_field_info( $field, $meta) {  
    $this->show_field_begin( $field, $meta );
	$plugin_path = $this->SelfPath;
	
    echo "<div class='info-block'><img src='{$plugin_path}/images/info.png' /><span class='at-info".( isset($field['class'])? ' ' . $field['class'] : '' )."' id='{$field['id']}'".( isset($field['style'])? "style='{$field['style']}'" : '' ).">{$field['desc']}</span></div>";
    //$this->show_field_end( $field, $meta );
  }

  public function addInfo($id,$args,$repeater=false){
    $new_field = array('type' => 'info','id'=> $id,'std' => '','desc' => '','style' =>'','name' => 'Info Field', 'class'=>'');
    $new_field = array_merge($new_field, $args);
    if(false === $repeater){
      $this->_fields[] = $new_field;
    }else{
   ...
   * @access public 
   */
  public function show_field_selectposts($field, $meta) {
    global $post;
    ...
    $posts = get_posts($options['args']);
      ...
      echo "<select title='Select' id='selected_posts' ".( isset($field['style'])? "style='{$field['style']}' " : '' )." name='{$field['id']}" . ($field['multiple'] ? "[]' multiple='multiple' style='height:auto'" : "'") . ">";
      foreach ($posts as $p) {
        echo "<option value='$p->ID'" . selected(in_array($p->ID, $meta), true, false) . ">$p->post_title</option>";
      }
      echo "</select>";
   ...
   * @access public
   */
  public function show_field_selecticon( $field, $meta ) {
    ...
    $this->show_field_begin( $field, $meta );
		foreach ( $field['options'] as $key => $value ) {
			...
			echo "<span class='icon-wrapper'>
				<input id='img_{$field['id']}_{$i}' type='radio' ".( isset($field['style'])? "style='{$field['style']}' " : '' )." class='at-radio img-radio {$key}".( isset($field['class'])? ' ' . $field['class'] : '' )."' name='{$field['id']}' value='{$key}'" . $checked . " /> <div class='dt-radio-label'>{$value}</div>
				<i class='fa {$value} icon-font-img {$selected}' onClick='document.getElementById(\"img_{$id}_{$i}\").checked = true;' title='{$value}'></i>
				</span>";
		}
    $this->show_field_end( $field, $meta );
   ...
   *  @param $repeater bool  is this a field inside a repeatr? true|false(default) 
   */
  public function addSelectIcon($id,$options,$args,$repeater=false){
    $new_field = array('type' => 'selecticon','id'=> $id,'std' => array(),'desc' => '','style' =>'','name' => 'Select Field','multiple' => false,'options' => $options);
    $new_field = array_merge($new_field, $args);
    if(false === $repeater){
      $this->_fields[] = $new_field;
    }else{
   ...
   *  @param $repeater bool  is this a field inside a repeatr? true|false(default)
   */
  public function addSelectPosts($id,$options,$args,$repeater=false){
    $post_type = isset($options['post_type'])? $options['post_type']: (isset($args['post_type']) ? $args['post_type']: 'post');
    $type = isset($options['type'])? $options['type']: 'select';
    $q = array('posts_per_page' => -1, 'post_type' => $post_type);
    if (isset($options['args']) )
      $q = array_merge($q,(array)$options['args']);
    $options = array('post_type' =>$post_type,'type'=>$type,'args'=>$q);
    $new_field = array('type' => 'selectposts','id'=> $id,'desc' => '','name' => 'Posts Field','options'=> $options,'multiple' => false);
    $new_field = array_merge($new_field, $args);
    if(false === $repeater){
      $this->_fields[] = $new_field;
    }else{
  ...
  public function addImageRadio($id,$options,$args,$repeater=false){
    $new_field = array('type' => 'imageradio','id'=> $id,'std' => array(),'desc' => '','style' =>'','name' => 'Radio Field','options' => $options);
    $new_field = array_merge($new_field, $args);
    if(false === $repeater){
      $this->_fields[] = $new_field;
    }else{
   ...
   */
  public function addImage($id,$args,$repeater=false){
    $new_field = array('type' => 'image','id'=> $id,'desc' => '','name' => 'Image Field', 'class'=> '', 'std' => array('id' => '', 'url' => ''),'multiple' => false);
    $new_field = array_merge($new_field, $args);
    if(false === $repeater){
      $this->_fields[] = $new_field;
    }else{
   ...
   *  @param $repeater bool  is this a field inside a repeatr? true|false(default) 
   */
  public function addPopupTextarea($id,$args,$repeater=false){
    $new_field = array('type' => 'popuptextarea','id'=> $id,'std' => '','desc' => '','style' =>'','name' => 'Textarea Field', 'class'=>'');
    $new_field = array_merge($new_field, $args);
    if(false === $repeater){
      $this->_fields[] = $new_field;
    }else{
  ...
  /**
   *  Add CheckboxListKey Field to meta box
   *  @author Ohad Raz
   *  @since 1.0
   *  @access public
   *  @param $id string  field id, i.e. the meta key
   ...
   *   which means the last param as false to get the values in an array
   */
  public function addCheckboxListKey($id,$options,$args,$repeater=false){
    $new_field = array('type' => 'checkbox_list_key','id'=> $id,'std' => '','desc' => '','style' =>'','name' => 'Checkbox List Field','options' =>$options,'multiple' => true,);
    $new_field = array_merge($new_field, $args);
    if(false === $repeater){
      $this->_fields[] = $new_field;
    }else{
 ...
 /**
   ...
   * @access public
   */
  public function show_field_image_solo( $field, $meta ) {
    wp_enqueue_media();
    $this->show_field_begin( $field, $meta );
    ...
    $value['url'] = isset($meta['src'])? $meta['src'] : $value['url']; //backwords capability
    $has_image    = empty($value['url'])? false : true;
    $w            = isset($field['width'])? $field['width'] : '200px !important';
    $h            = isset($field['height'])? $field['height'] : 'auto';
    $PreviewStyle = "style='width: $w; height: $h;". ( (!$has_image)? "display: none;'": "'");
    $id           = $field['id'];
    $multiple     = isset($field['multiple'])? $field['multiple'] : false;
   ...
   *  @param $repeater bool  is this a field inside a repeatr? true|false(default) 
   */
  public function addImageSolo($id,$args,$repeater=false){
    $new_field = array('type' => 'image_solo','id'=> $id,'desc' => '','name' => 'Image Field','std' => array('id' => '', 'url' => ''),'multiple' => false);
    $new_field = array_merge($new_field, $args);
    if(false === $repeater){
      $this->_fields[] = $new_field;
    }else{
